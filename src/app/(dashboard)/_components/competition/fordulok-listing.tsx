...
import ForduloCloseDialog from "./ongoing/fordulo-close-dialog";
import ForduloStartForm from "./upcoming/fordulo-start-form";
...
export function FordulokListing({
    category,
    stages,
    students,
    isAdmin = false,
    ...
    students: ({ student: Student } & StudentCategory)[];
    isAdmin?: boolean;
}) {
        ...
        return (
            <div className="text-center py-10">
                ...
                <p className="text-sm text-muted-foreground mt-2 mb-6">
                    Ehhez a kategóriához még nem tartoznak fordulók.
                </p>
                {isAdmin && (
                    <CreateStageDialog
                        category={category}
    ...
    const selectedRound =
        stages.find((stage) => stage.id === selectedRoundId) || stages[0];
                        ...
                        <CardContent>
                            ...
                            >
                                ...
                                <TabsContent value="overview" className="mt-4">
                                    <div className="space-y-4">
                                        <div className="flex flex-col pt-4 border-t">
                                            ...
                                            {selectedRoundId && (
                                                <TaskFiles
                                                    stageId={selectedRoundId}
                                                    files={selectedRound.files}
                                                />
                                            )}
                                        </div>
                                        <div className="flex flex-col gap-4 border-t pt-4">
                                            ...
                                            <Link
                                                href={`/admin/versenyek/${category.competitionId}/kategoria/${category.id}/fordulo/${selectedRound.id}/task`}
                                            >
                                                <Button
                                                    variant="outline"
                                                    className="w-fit"
                                ...
                                <TabsContent value="students" className="mt-4">
                                    <ForduloStudentsTable
                                        students={selectedRound.students}
                                    />
                                </TabsContent>
                                {isAdmin && (
                                    <TabsContent
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex flex-col pt-4 border-t">
                                                {selectedRound.status ===
                                                    "UPCOMING" && (
                                                    <ForduloUpcoming selectedRound={selectedRound} />
                                                )}
                                                {selectedRound.status ===
                                                    "ONGOING" && (
                                                    <ForduloOngoing selectedRound={selectedRound} />
                                                )}
                                                {selectedRound.status ===
                                                    "FINISHED" && (
                                                    <ForduloFinished selectedRound={selectedRound} />
                                                )}
                                            </div>
                                        </div>
                                    </TabsContent>
