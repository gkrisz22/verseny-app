    ...
    <header className="border-b bg-background sticky top-0 z-10">
      <div className="container mx-auto px-4 flex h-16 items-center justify-between">
        <div className="flex items-center gap-6">
          <TeamSwitcher teams={[{ name: "Szervezet 1", logo: Trophy, role: "Admin" },
                        ...
                        .map((child) => (
                          <DropdownMenuItem key={child.href} asChild>
                            <Link
                              href={child.href}
                              className={cn(
                                pathname.startsWith(child.href) && "bg-primary/10 text-primary font-medium",
                              )}
                            >
                              {child.label}
                            </Link>
      ...
      <div className="md:hidden border-t">
        <nav className="flex overflow-x-auto">
          {navItems.map((item) => {
            ...
            const mobileHref = item.children && item.children.length > 0 ? item.children[0].href : item.href
