...
export async function createStudent(
    prevState: ActionResponse<CreateStudentDTO>,
    formData: FormData
): Promise<ActionResponse<CreateStudentDTO>> {
    return actionHandler<CreateStudentDTO>(
        createStudentSchema,
        formData,
        async (data) => {
            try {
                ...
                if (!orgData) {
                    throw new Error(
                        "Hiba történt a tanuló létrehozása közben: Ön nem tagja egy szervezetnek sem!"
                    );
                }
                ...
                if (!currentYear) {
                    throw new Error(
                        "Hiba történt a tanuló létrehozása közben: Nincs jelenlegi tanévre vonatkozó beállítás!"
                    );
                }
                ...
                if (data.uniqueId && !data.id) {
                    ...
                    if (students.length === 0) {
                        throw new Error(
                            "Nem létezik diák a megadott azonosítóval!"
                        );
                    }
                ...
                return {
                    ...
                    message:
                        e instanceof Error
                            ? e.message
                            : "Hiba történt a tanuló" +
                              (data.id ? " frissítése" : " létrehozása") +
                              " közben",
                    data: {
                        name: data.name,
                        grade: data.grade,
            ...
            return {
                success: true,
                message: data.id
                    ? "Diák sikeresen frissítve"
                    : "Diák sikeresen létrehozva",
            };
        }
    );
}
...
export async function deleteStudent(id: string): Promise<boolean> {
    try {
        ...
        if (!orgData) {
            throw new Error(
                "Hiba történt a tanuló törlése közben: Ön nem tagja egy szervezetnek sem!"
            );
        }
        const schoolId = orgData.id;
        ...
        logger.error(
            "[deleteStudent]" +
                (e instanceof Error
                    ? e.message
                    : "Hiba történt a tanuló törlése közben")
        );
        return false;
    }
    return true;
...
export async function signStudentsUpForCategory(
    categoryId: string,
    studentIds: string[]
) {
    try {
        ...
        if (!orgData) {
            throw new Error(
                "Hiba történt a jelentkezés során: Ön nem tagja egy szervezetnek sem!"
            );
        }
        const schoolId = orgData.id;
        ...
        const ok: string[] = [];
        const categoryStudents = await categoryService.getStudentsInCategory(
            categoryId
        );

        for (const student of students) {
            if (
                category?.eligibleGrades.length === 0 ||
                category?.eligibleGrades.includes(student.grade || 0)
            ) {
                if (
                    categoryStudents?.students.find(
        ...
        revalidatePath(
            `/org/versenyek/${category?.competitionId}/aktualis/${categoryId}/reszletek`
        );
        return {
            success: true,
